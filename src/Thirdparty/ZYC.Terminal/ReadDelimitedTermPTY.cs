...
{
    protected int curBufferOffset; //where in the entirebuffer does the current buffer to read into start
    protected char[] delimiter;
    protected TimeSpan delimiterTimeout;

    protected int
        lastDelimEndOffset; //where in the entirebuffer did the last delimiter end should always be <= curBufferOffset, the data between here and curBufferOffset is what is still valid data needing to be sent.
    ...
    ///     Maximum time to buffer output waiting for a delimiter since the last delimiter
    ///     was seen. Once this time passes the entire buffer is sent on the next output.
    /// </param>
    public ReadDelimitedTermPTY(int READ_BUFFER_SIZE = 1024 * 16, bool USE_BINARY_WRITER = false,
        ReadOnlySpan<char> delimiter = default, TimeSpan MaxWaitTimeoutForDelimiter = default) : base(READ_BUFFER_SIZE,
        USE_BINARY_WRITER)
    {
        ...
        if (state.curBuffer.Length == 0)
        {
            ...
            else
            {
                ...
                while (toCopyBlocks > 0)
                {
                    ...
                    var copyAmount = copyPos + copyWindow > state.entireBuffer.Length
                        ? state.entireBuffer.Length - copyPos
                        : copyWindow;
    ...
    /// <summary>
    ///     Will only send data to the UI Terminal after each delimiter is hit. Caution as the terminal will not get any
    ///     updates until the delimiter is hit. Pass a 0 length span to disable waiting for a delimiter.   Note if the buffer
    ///     is completely filled before the delimiter is hit the buffer will be pasesd on.
    ///     Note: Delimiter itself is not ever passed
    /// </summary>
    /// <param name="delimiter"></param>
    /// <param name="MaxWaitForDelimiter">
    ///     Maximum time to buffer output waiting for a delimiter since the last delimiter was
    ///     seen. Once this time passes the entire buffer is sent on the next output.
    /// </param>
